...
let MailService = MailService_1 = class MailService {
    ...
    constructor() {
        ...
        this.resend = new resend_1.Resend(apiKey || 're_placeholder');
    ...
    async sendWelcome(user) {
        ...
        await this.send(user.email, 'Bienvenue sur GoConnexions 🚀', this.wrap(`
    ...
    async sendEmailVerification(user, token) {
        ...
        await this.send(user.email, 'Confirme ton adresse email — GoConnexions', this.wrap(`
        <h1 style="margin:0 0 16px;font-size:24px;font-weight:700;color:#0f172a">
          Confirme ton email
        </h1>
        <p style="margin:0 0 16px;color:#475569">
    ...
    async sendPasswordReset(user, token) {
        ...
        await this.send(user.email, 'Réinitialisation de ton mot de passe — GoConnexions', this.wrap(`
        <h1 style="margin:0 0 16px;font-size:24px;font-weight:700;color:#0f172a">
    ...
    async sendSubscriptionActivated(user, plan, periodEnd) {
        const endDate = periodEnd.toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' });
        const dashboardUrl = `${process.env.FRONTEND_URL ?? 'http://localhost:3000'}/dashboard`;
        await this.send(user.email, `Abonnement ${plan} activé — GoConnexions`, this.wrap(`
    ...
    async sendSubscriptionCancelled(user, plan, periodEnd) {
        const endDate = periodEnd.toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' });
        const pricingUrl = `${process.env.FRONTEND_URL ?? 'http://localhost:3000'}/pricing`;
        await this.send(user.email, `Abonnement ${plan} annulé`, this.wrap(`
        <h1 style="margin:0 0 16px;font-size:24px;font-weight:700;color:#0f172a">
          Abonnement annulé
        </h1>
        <p style="margin:0 0 16px;color:#475569">
    ...
    async sendPaymentFailed(user, plan) {
        const checkoutUrl = `${process.env.FRONTEND_URL ?? 'http://localhost:3000'}/billing/checkout`;
        await this.send(user.email, '⚠️ Échec de paiement — GoConnexions', this.wrap(`
        <div style="text-align:center;margin-bottom:24px">
    ...
    async sendWiseRenewalReminder(user, amount, currency, reference, periodEnd, accountDetails) {
        const endDate = periodEnd.toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' });
      ...
      `)
            .join('');
        await this.send(user.email, `Renouvellement de ton abonnement — Réf. ${reference}`, this.wrap(`
        <h1 style="margin:0 0 16px;font-size:24px;font-weight:700;color:#0f172a">
    ...
    async sendBusinessCardInvitation(senderName, recipientEmail) {
        const signupUrl = `${process.env.FRONTEND_URL ?? 'http://localhost:3000'}/auth/signup`;
        await this.send(recipientEmail, `${this.esc(senderName)} vous envoie sa carte de visite — GoConnexions`, this.wrap(`
        <h1 style="margin:0 0 16px;font-size:24px;font-weight:700;color:#0f172a">
